...
*Hellcard modding tools* is a special editor that helps with creating and editing content for the game. It gives you access to the hierarchy of the game and a lot of specialized tools.
...
The UI of *Hellcard modding tools* is separated into a few windows. Thay gain focus when you click on them. Shortcuts and navigation with keyboard works only on windows with focus.  

![UI](./content/mods_tool_ui.png)  

### 1. Object tree

- Hierarchy of all objects loaded in game
...
- Different types of objects have different options that can be used from the context menu when you right click on them. Some of these options are deprecated and should not be used. **Make sure to save and add objects using methods mentioned in documentation about specific types of objects**
- Some objects have specific debug options and functions (more [here](/docs/CheatSheet.md))
- You can navigate the *Object tree* using mouse or arrow keys
- You can spawn objects (cards, monsters, artefacts, ...) in dungeon and lobby from managers by double clicking them (or clicking *Enter* on them) in *Object tree*
- A lot of objects in tree can be copied and pasted using context menu or *CTRL+C*, *CTRL+V*
- There is search function that can find objects in tree by name
    - To show search window use *CTRL+F*
...
- Some of the message types can be hidden by default. You should make sure to display all *Error* messages (red ones), and pay attention to them, because they usually tell you what should be done to prevent them, like for example set some important parameter
- Especially useful when writing angelscripts. You can log messages from your scripts, which can be very helpful when debugging them